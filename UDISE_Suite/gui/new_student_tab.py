...
from browser.new_student_automation import UdiseNewStudentAutomation

class NewStudentTab(ctk.CTkFrame):
    ...
    def _scan_page(self):
        ...
        def task():
            found_fields = self.automation.scan_page_fields(self.current_student, progress_callback=self.update)
            self._update_comparison_table(found_fields)
            self.btn_scan.configure(state="normal")
    ...
    def _write_data(self):
        ...
        def task():
            filled_fields = self.automation.fill_visible_fields(self.current_student, progress_callback=self.update)
